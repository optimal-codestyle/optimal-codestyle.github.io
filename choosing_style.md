...
Кроме того, поскольку в стиле _Allman_ скобки располагаются на отдельных строках, блок кода внутри них визуально явно отделен от заголовка конструкции, что лучше отражает структуру этой конструкции. В _1TBS_ такого явного разделения нет, особенно в случае, когда предшествующая часть занимает несколько строк.
...
Однако, поиск открывающей скобки, отличается от чтения текста, поскольку мы имеем определенные ожидания относительно её расположения (вверху и в конце строки), а сама она представляет простой визуальный объект. Поэтому, например, в случае, когда скобки разделены по вертикали лишь одной строкой, такое смещение может оказаться размера области распознавания и, при этом не требовать больших ментальных усилий при чтении.

Наверно, можно сказать, что величина этого смещения обратно пропорциональна количеству строк, разделяющему парные скобки.
