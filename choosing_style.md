...
**Рисунок 16. Тест с прищуриванием для кода на рисунке 15.**
...
**Рисунок 22. Однострочное форматирование**
...
**Рисунок 23. Выравнивание пар метка:значение**
...
**Рисунок 25. Вариант 1: метки и аргументы выровнены влево по отдельности**
...
**Рисунок 26. Вариант 2: внутреннее выравнивание**
...
На сегодняшний день в языках с Си-подобным синтаксисом существуют два основных способа расстановки фигурных скобок, в одном из которых открывающая скобка находится на отдельной строке с тем же отступом, как и начало связанного с ней предшествующего синтаксического элемента, а во втором открывающая скобка располагается в конце строки, содержащей окончание такого элемента.

Далее я буду условно называть эти стили _Allman_ и _One Truce Brace Style_ (_1TBS_) по названиям наболее популярных стилей, которые используют соответсвующие правила расстановки скобок. В стиле _1TBS_, если за закрывающей скобкой следует продолжение синтаксичекой конструкции (как например, в случае `if`/`else`), то это продолжение располагается на той же строке, что и закрывающая скобка.

Расположение открывающей скобки в начале отдельной строки в стиле _Allman_ обладает следующими преимуществами:

1. Скобка всегда располагается в левой части визуальной области, т. е. в области наибольшего внимания, и при сканировании кода (в котором преобладают вертикальные движения глаз) всегда так или иначе попадают в область фокального зрения. В стиле _1TBS_ открывающие скобки часто оказываются в правой области кода и попадают лишь в область периферийного зрения, что значительно затрудняет их обнаружение.

2. Облегчено сопоставление открывающей и закрывающей скобок и, соответственно, определение границ обрамляемого ими блока кода. Поиск парной скобки требует лишь вертикального перемещения взгяда, на пути его следования от одной скобки к другой нет никакого текста, и поиск происходит в известном направлении до первого символа.

   Всегда, когда мы видим в тексте закрывающую скобку на отдельной строке, мы знаем, что парная ей открывающая находится *выше*, а это значит, что основным и естественным направлением поиска её будет поиск вверх. Таким образом, стиль _Allman_ максимальным образом оптимизует поиск парной скобки, по сравнению со стилем _1TBS_, который в общем случае требует больших усилий за счет того, что поиск осуществляется в широком секторе, причем взгляд проходит через текст, который надо анализировать, и который часто содержит вложенные пары фигурных скобок, визуально конкурирующие с целевой.

3. Скобка расположена в начале строки, именно там, где мы ожидаем увидеть начало чего бы то нибыло, и естественным образом обозначает начало блока кода. В стиле _1TBS_ из-за своего положения в конце строки открывающая скобка часто перестает играть роль явного визуального маркера начала блока, в то время как в некоторых случаях (напр. повторяющиеся конструкции `} else if`) закрывающая скобка предыдущего блока оказывается в начале строки, содержащей конструкцию, предваряющую новый блок, и тем самым визуально связывется с началом этого блока. Таким образом, её формальное и визуальное значения не соотвествуют друг другу.   

4. Расположение скобок на отдельных строках, естественным образом добавляет вертикальные пробелы между синтаксической конструкцией перед открывающей скобкой и обрамляемым болком кода, что, в большинстве случаев, лучше отражает структуру всей конструкции.

В то же время, существуют ситуации, когда стиль _1TBS_  оказывается предпочтительнее. Это может быть вызвано тем, что в каком-то случае дополнительное вертикальное пространство между различными частями синтаксической конструкции выглядит слишком большим по сравнению с размером отдельных ее частей, и вся конструкция становится визуально разрозненной и перестает восприниматься как единое целое. При этом, несмотря на то, что при использовании _1TBS_ смещение открывающей скобки вправо от оптимального положения в общем приводит к возрастанию сложности её обнаружения, это смещение может оказаться небольшим, для того чтобы оказать какое-либо влияние на восприятие кода, либо обнаружение открывающей скобки оказывается непринципиальным (например, в случае конструкции `if`, когда и условие и блок кода занимают по одной строке).

 <!--смещение, чтобы быть приемлимым и не вызывать заметных затруднений в восприятии программы. Возможно, в качестве приблизительной оценки здесь будут также актуальны размеры областей идентификации и распознавания, упомянутые выше, и составляющие 7-8 и 14-15 размеров букв справа о точки фиксации, соответственно. Поиск открывающей скобки отличается от чтения текста в том, что мы имеем определенные ожидания относительно её расположения (вверху и в конце строки), а сама она представляет простой визуальный объект. Поэтому, например, в случае, когда скобки разделены по вертикали лишь одной строкой, такое смещение может оказаться больше размера области распознавания и, при этом не требовать больших ментальных усилий при чтении. Наверно, можно сказать, что величина этого смещения обратно пропорциональна количеству строк, разделяющему парные скобки.-->

Таким образом, отдавая предпочтение стилю _Allman_, хочется заметить, что комбинация обоих стилей позволяет формировать нужный ритмический рисунок кода, и выбор того или иного стиля должен осуществляться программистом в каждом конкретном случае для того, чтобы сформировать оптимальное визуальное представление соответствующей части кода.

<!--Например, в случае короткого условия в операторе `if`, когда код внутри скобок содержит 1-3 строки, разделение заголовка и тела конструкции посредством размещения открывающей скобки на отдельной строки может оказаться очень сильным из-за малого размера области кода, и приводить к развалу визуальной структуры кода, так что заголовок конструкции, оказывается недостаточно связанным с кодом внутри скобок по сравнению с окружающими элементами.-->
