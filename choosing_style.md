...
То есть, отавая предпочтение стилю _Allman_, необходимо понимать, что выбор того или иного сталя должен осуществляться программистом в каждом конкретном случае для того, чтобы сформировать оптимальное визуальное представление соответствующей части программного кода. <!-- В общем случае, можно предложить следующее правило: использование _1TBS_ стиля возможно, когда заголовок утверждения, предшествующий '{' занимает одну строку в левой части страницы текста и её окончание не сливается с окончанием первой строки обрамляемого кода, и этот код не содержит более трех строк. -->
