...
**Части траекторий движения глаз при корректном опознании underscore и camelCase идентификаторов, состоящих из трех слов[⁴](#4)**
<img src="https://habrastorage.org/webt/2-/dz/vt/2-dzvtxei7st1fmgikp2ww6p4zw.jpeg" />
<!-- <img src="https://habrastorage.org/webt/ir/h8/ww/irh8ww1o_elwbchg2vrg8zbgnme.png" /> -->
...
Всегда, когда мы видим в тексте закрывающую скобку на отдельной строке, мы знаем, что парная ей открывающая находится *выше*, а это значит, что основным и естественным направлением поиска её будет поиск вверх. С этой точки зрения стиль _Allman_ предпочтительнее, поскольку в нем открывающая скобка располагается на оптимальной для поиска позиции – прямо над закрывающей скобкой, то есть там, где мы её и ожидаем увидеть. При этом поиск открывающей скобки требует лишь вертикального перемещения глаз, на пути следования взгляда от одной скобки к другой нет никакого текста, и поиск происходит в известном направлении до первого символа. Стиль _1TBS_ в общем случае может потребовать больших усилий за счет того, что поиск осуществляется в широком секторе, причем взгляд проходит через текст, который надо анализировать, и который часто содержит вложенные пары фигурных скобок.

В стиле _Allman_ скобки располагаются на отдельных строках, и блок кода внутри них визуально явно отделен от заголовка конструкции, что лучше отражает структуру всей конструкции. В _1TBS_ такого явного разделения нет, особенно в случае, когда предшествующая часть занимает несколько строк.
...
В то же время, несмотря на то, что смещение открывающей скобки вправо от оптимального положения приводит к возрастанию сложности её обнаружения, можно предположить, что это происходит не скачкообразно, и небольшое смещение не окажет заметного влияния. Возникает вопрос, насколько «небольшим» может быть это смещение, чтобы быть приемлимым и не вызывать заметных затруднений в восприятии программы. Возможно, в качестве приблизительной оценки здесь будут также актуальны размеры областей идентификации и распознавания, упомянутые выше, и составляющие 7-8 и 14-15 размеров букв справа о точки фиксации. 

Однако, поиск открывающей скобки, отличается от чтения текста, поскольку мы имеем определенные ожидания относительно её расположения (вверху и в конце строки), а сама она представляет простой визуальный объект. Поэтому, например, в случае, когда скобки разделены по вертикали лишь одной строкой, такое смещение может оказаться больше размера области распознавания и, при этом не требовать больших ментальных усилий при чтении. Наверно, можно сказать, что величина этого смещения обратно пропорциональна количеству строк, разделяющему парные скобки.

Дополнение стиля _Allman_ стилем _1TBS_ позволяет формировать нужный ритмический рисунок кода. Например, в случае короткого условия в операторе `if`, когда код внутри скобок содержит 1-3 строки, разделение заголовка и тела конструкции посредством размещения открывающей скобки на отдельной строки может оказаться очень сильным из-за малого размера области кода, и приводить к развалу визуальной структуры кода, так что заголовок конструкции, оказывается недостаточно связанным с кодом внутри скобок по сравнению с окружающими элементами.

Таким образом, отдавая предпочтение стилю _Allman_, необходимо понимать, что выбор того или иного стиля должен осуществляться программистом в каждом конкретном случае для того, чтобы сформировать оптимальное визуальное представление соответствующей части программного кода.
