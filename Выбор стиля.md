...
Тем не менее, сами правила часто вызывают вопросы, поскольку критерии их выбора неясны и иногда просто противоречат аналогичным правилам в других подобных стилях.
...
Гештальтпсихологи также сформулировали 6 принципов перцептивной организации. В сответствии с этими принципами _объекты, которые_ 
...
<!--(Популярный у разработчиков пользовательского интерфейса "тест с прищуриванием" (squint test) )-->
...
Имеет смысл провести тест, аналогичный популярному у разработчиков пользовательского интерфейса "тесту с прищуриванием" (squint test), смысл которого заключается в том, чтобы попробовать оценить расфокусированное изображение разработанного интерфейса (текста программы в нашем случае). Мы отключим подсветку синтаксиса для того, чтобы исключить группировку по фактору сходства по окраски, которую мы не контролируем:
...
Из всех предстваленных до сих пор вариантов визуального представления, этот вариант наиболее точно передает логическую структуру кода. В нем явно различимы области меток и значений, идентификация и поиск отдельных элементов внутри их значительно облегчается. Однако из-за большой разницы в длинах меток аргументов связи внутри пар с короткими метками оказываются ослабленными.
...
Отсутствие жесткого ограничения справа, тем не менее, не озачает, что ограничения нет. Многовековой опыт книгопечатания и десятилетия опыта, наработанного [веб-дизайнерами](http://webtypography.net/2.1.2), сходятся к тому, что оптимальная длина строки, обеспечивающей комфортное чтение составляет приблизительно 45-75 символов, независимо ни от размера шрифта, ни от наличия дополнительного пространства, позволяющего разместить более длинную строку. 
...
[^11]: Альфред Лукьянович Ярбус. Роль глаз в процессе зрения. М.:Наука, 1965
